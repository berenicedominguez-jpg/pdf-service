...
def build_docx_pdf(body_xml, tmpdir, nombre, imagenes=None, evidencias_pdf=None):
    ...
    for i, page in enumerate(reader_main.pages):
        ...
        if page_num == 2 and mapa:
            ...
            # Calcular posición Y donde termina la barra LUGAR DEL EVENTO
            # Encabezado ~100pts + tabla descripción ~182pts + sep ~5pts + barra ~22pts = ~309pts desde arriba
            # En coords ReportLab (0=abajo): 842 - 309 = 533pts
            TEXT_BOTTOM_Y = 309  # pts desde arriba donde termina la barra LUGAR DEL EVENTO
            IMG_TOP_Y = H - TEXT_BOTTOM_Y  # en coords reportlab
            AVAIL_H_MAPA = IMG_TOP_Y - 20  # espacio hasta margen inferior
